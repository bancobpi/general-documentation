...
- **Focus on representing information from the source application**: Attributes should accurately mirror the intrinsic data of the resource, ensuring a faithful reflection of the source's information, without preemptively considering the perspective of the consumer, as this could introduce bias or compromise the authenticity of the data. The consumers can then interpret and manipulate the data in a manner consistent with its original context.
...
| 206             | "Partial Content"<br>The server has fulfilled the partial GET request for the resource. E.g. downloading a file. | YES | | GET              |
